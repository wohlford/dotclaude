...
# Script: env-claims-check-test.sh
# Purpose: PostToolUse hook — run the env-claims-check test suite when the checker changes
...
#   2 — env-claims-check tests failed (stderr fed back to Claude to fix)
#
# Global hook: fires on every Edit|Write in every repo, so guards run cheapest-first and exit 0
# fast for anything that is not env-claims-check.py (or its suite) in a repo that actually carries
# the suite.
#
# NOTE this hook takes its target from the JSON payload on stdin and IGNORES argv entirely. Run
# as a CLI (`env-claims-check-test.sh some/file`) it examines nothing: with stdin at EOF it exits
# 0, which reads exactly like a clean pass, and with stdin inherited it blocks forever. To drive
# it by hand, feed it a payload:
#   printf '{"tool_input":{"file_path":"%s"}}' "$PWD/scripts/env-claims-check.py" | \
#     bash scripts/env-claims-check-test.sh
...
case "$file_path" in
  */scripts/env-claims-check.py | \
    */scripts/tests/test_env_claims_check.py) ;;
  *) exit 0 ;;
esac
...
if [[ ! -f "$root/scripts/tests/test_env_claims_check.py" ]]; then
  if grep -q 'dotclaude-test-runner-hook' "$root/scripts/$(basename "$0")" 2>/dev/null; then
    printf '%s\n' \
      "MISSING SUITE: scripts/tests/test_env_claims_check.py is absent, but this repo owns $(basename "$0") — the gate did NOT run." \
...
output=$(cd "$root" && python3 -m pytest scripts/tests/test_env_claims_check.py -q \
  --no-header 2>&1) && rc=0 || rc=$?

if [[ "$rc" -ne 0 ]]; then
  printf 'env-claims-check tests FAILED after editing %s:\n' "$file_path" >&2
  printf '%s\n' "$output" | tail -20 >&2
  exit 2
fi

printf 'env-claims-check tests passed: %s\n' "$(printf '%s\n' "$output" | tail -1)"
exit 0
