...
# Script: memory-index-check-test.sh
# Purpose: PostToolUse hook — run the memory-index-check test suite when the checker changes
...
#   2 — memory-index-check tests failed (stderr fed back to Claude to fix)
#
# Global hook: fires on every Edit|Write in every repo, so guards run cheapest-first
# and exit 0 fast for anything that is not memory-index-check.py (or its test) in a repo
...
case "$file_path" in
  */scripts/memory-index-check.py|*/scripts/tests/test_memory_index_check.sh) ;;
  *) exit 0 ;;
esac
...
if [[ ! -f "$root/scripts/tests/test_memory_index_check.sh" ]]; then
  if grep -q 'dotclaude-test-runner-hook' "$root/scripts/$(basename "$0")" 2>/dev/null; then
    printf '%s\n' \
      "MISSING SUITE: scripts/tests/test_memory_index_check.sh is absent, but this repo owns $(basename "$0") — the gate did NOT run." \
...
output=$(cd "$root" && bash scripts/tests/test_memory_index_check.sh 2>&1) && rc=0 || rc=$?
...
printf 'memory-index-check tests passed: %s\n' "$(printf '%s\n' "$output" | tail -1)"
exit 0
