...
def _detect_lint_drift(
    existing_body: list[str], expected_body: list[str], directives: dict
) -> str | None:
...
def process_file(
    ...
    for idx, block in enumerate(doc.blocks):
        ...
        if is_lint:
            ...
            if drift:
                lint_drifts.append(
                    f"  {block.handler} (line {block.open_line}): {drift}"
                )
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    for md in md_files:
        try:
            ...
            return 2
        # A file whose only markers are BROKEN parses to zero blocks but non-empty
        # errors — it is still a marker file. Counting it prevents the "no markers
        # found" early return from silently swallowing the errors (and exiting 0).
        if block_count > 0 or errors:
            marker_files += 1
        for err in errors:
    ...
    if marker_files == 0:
        if not args.check:
            ...
            print(
                "  /sync-docs init                # scaffold READMEs for content dirs"
            )
            print(
                "  /sync-docs add skills          # add a marker for the skills index"
            )
        return 0
...
def _choose_marker(
    dirs: list[Path], date_dirs: list[Path], files: list[Path], md_files: list[Path]
) -> str:
...
def cmd_init(args: argparse.Namespace) -> int:
    """Propose (and on confirmation scaffold) READMEs for qualifying content dirs."""
    repo_root = discover_repo_root(args.scope)
    config = load_project_config(repo_root)
    ...
    for d in _walk_dirs(repo_root):
        ...
        rel_str = str(rel) + "/"
        if any(
            rel_str.startswith(p.rstrip("/") + "/") or str(rel) == p.rstrip("/")
...
def cmd_add(args: argparse.Namespace) -> int:
    """Append an empty marker block for the named handler to the target file."""
    ...
    if handler == "custom":
        ...
        print(
            f"unknown handler: {handler!r}; choices: {', '.join(list(HANDLER_DEFAULT_MARKERS) + ['custom'])}",
            file=sys.stderr,
        )
        return 2

    target = (
        Path(args.into).resolve() if args.into else Path.cwd().resolve() / "README.md"
    )
    block = f"\n{open_marker}\n{close_marker}\n"
    ...
    if target.exists():
        ...
        title = (
            target.parent.name.replace("_", " ").replace("-", " ").title() or "Index"
        )
...
def main(argv: list[str] | None = None) -> int:
    """CLI entry point: parse arguments and dispatch to sync/init/add."""
    ...
    init_p = subparsers.add_parser(
        "init", help="Scaffold READMEs for content directories"
    )
    init_p.add_argument(
        "--yes-to-all", action="store_true", help="Skip confirmation prompts"
    )
    init_p.add_argument(
        "--max-depth",
        type=int,
    ...
    add_p.add_argument(
        "handler",
        help="Handler name (skills, agents, plugins, hooks, scripts, index, custom)",
    )
