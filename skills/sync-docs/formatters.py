...
class ColSpec:
    """Specification for one Markdown table column."""
...
def _escape_cell(s: str) -> str:
    ...
    - '<!--' is defused to '&lt;!--': it cannot escape the sync region (markers are
      whole-line-anchored), but GitHub would render the comment invisibly and
      silently hide the rest of the cell in the public README.
    """
    s = s.replace("\r\n", " ").replace("\n", " ").replace("\r", " ")
    s = s.replace("|", "\\|")
    s = s.replace("<!--", "&lt;!--")
...
def render_table(rows: list[dict[str, str]], cols: list[ColSpec]) -> list[str]:
    ...
    escaped_rows: list[dict[str, str]] = [
        {c.name: _escape_cell(row.get(c.name, "")) for c in cols} for row in rows
    ]
    ...
    header_cells = [c.name.ljust(w) for c, w in zip(cols, widths, strict=True)]
    ...
    for row in escaped_rows:
        cells = [
            row.get(c.name, "").ljust(w) for c, w in zip(cols, widths, strict=True)
        ]
...
def parse_table(lines: list[str]) -> tuple[list[ColSpec], list[dict[str, str]]]:
    ...
    for ln in table_lines[2:]:
        ...
        if len(cells) < len(header):
            continue  # too few columns — genuinely malformed, skip
        if len(cells) > len(header):
            # Extra cells mean an UNescaped '|' inside a cell — typically a hand-edited manual
            # column (usually the last). Coalesce the overflow back into the final column (with a
            # literal '|') so the manual value is preserved and round-trips, rather than the whole
            # row being silently dropped and the value blanked on the next sync.
            keep = len(header) - 1
