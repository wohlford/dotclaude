...
def test_render_table_idempotent_with_canonical_padding():
    ...
    cols = [
        formatters.ColSpec(name="A", role="key"),
        formatters.ColSpec(name="B", role="auto"),
    ]
...
def test_render_table_defuses_html_comment_opener():
    # '<!--' in a cell can't escape the sync region, but rendered raw it would
    # hide the rest of the cell as an HTML comment in the public README.
    rows = [{"A": "before <!-- hidden after"}]
    cols = [formatters.ColSpec(name="A", role="key")]
    out = formatters.render_table(rows, cols)
    assert "<!--" not in out[2]
    assert "&lt;!--" in out[2]
...
def test_parse_table_preserves_unescaped_pipe_in_last_cell():
    # A hand-typed raw '|' in a manual cell must not silently drop the row / blank the value.
    cols, rows = formatters.parse_table(["| A | B |", "| :- | :- |", "| x | a | b |"])
    assert rows == [{"A": "x", "B": "a | b"}]


def test_parse_table_still_skips_too_few_cells():
    cols, rows = formatters.parse_table(["| A | B |", "| :- | :- |", "| only |"])
    assert rows == []
