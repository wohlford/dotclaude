...
def test_parse_quoted_directive():
  text = '<!-- sync:agents cols=Agent:key,"Used by":manual -->\n<!-- /sync:agents -->\n'
  doc = markers.parse(text)
  assert doc.blocks[0].directives == {'cols': 'Agent:key,Used by:manual'} or \
         doc.blocks[0].directives == {'cols': 'Agent:key,"Used by":manual'}
