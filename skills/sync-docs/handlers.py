...
from formatters import ColSpec, parse_table, render_table
...
@dataclass
class Source:
    """One discovered source file plus the metadata extracted from it."""
...
def _sort_rows(
    rows: list[dict[str, str]], directive: str | None, key_field: str
) -> list[dict[str, str]]:
...
class SkillsHandler:
    ...
    def discover(
        ...
        globs = (
            [cfg_source]
            if cfg_source
            else ["skills/*/SKILL.md", ".claude/skills/*/SKILL.md"]
        )
        sources: list[Source] = []
        seen: set[Path] = set()
        for g in globs:
    ...
    def render(
        ...
        key = _get_key_col(cols)
        rows = _sort_rows(
            rows, directives.get("sort"), key.name if key else cols[0].name
        )
        ...
        for s in sources:
            ...
            for c in cols:
                if c.name == "Event":
                    ...
                    row[c.name] = (
                        f"`{s.fields['matcher']}`" if s.fields.get("matcher") else ""
                    )
                elif c.name == "Script":
                    row[c.name] = (
                        f"`{s.fields['script']}`" if s.fields.get("script") else ""
                    )
                elif c.name == "Purpose":
...
class IndexHandler:
    ...
    def render(
        ...
        if mode == "mtime":
            sources = sorted(
                sources, key=lambda s: s.path.stat().st_mtime, reverse=desc
            )
        else:
...
def get_handler(name: str) -> Handler | None:
    """Look up a built-in handler by name; None if unknown."""
