...
4. Write `skills/<name>/SKILL.md` with the frontmatter and section stubs shown below. Substitute `<name>`, `<one-line description>`, and `<Subject>` with the provided values; leave every other bracketed placeholder in the body verbatim as an authoring prompt — do not invent Process steps or Rules from the one-line description; the authoring pass fills those in before `/vet`.
