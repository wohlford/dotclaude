...
def test_exit_parity_state_neutral(tmp_path):
    ...
    t = script(
        tmp_path, 'f="$TMPDIR/flag"; if [ -e "$f" ]; then exit 1; fi; touch "$f"\n'
    )
    assert run_it(target=t).returncode == 1
...
def test_symlink_repoint_nonidempotent(tmp_path):
    t = script(
        tmp_path,
        "if [ -e link ]; then ln -sfn /tmp/o link; else ln -sfn /etc/hosts link; fi\n",
    )
    assert run_it(target=t).returncode == 1
...
def test_git_divergence_diff_excludes_dotgit(tmp_path):
    # A git-only divergence must NOT dump binary .git internals into the failure diff:
    # zlib-compressed git objects emit non-UTF8 bytes that flakily crash text-mode capture.
    # The verdict comes from the porcelain manifest, so the diff excludes .git like manifest().
    t = script(
        tmp_path,
        "git -c user.name=t -c user.email=t@t -c commit.gpgsign=false "
        "commit --allow-empty -q -m x\n",
    ...
    r = subprocess.run(
        ["bash", str(HELPER), "--git", "--", str(t)],
        capture_output=True,  # bytes on purpose: output must not be assumed decodable
        timeout=60,
    )
    assert r.returncode == 1
    assert b"/.git/" not in r.stdout
    assert b"/.git/" not in r.stderr
...
def test_containment_real_home_untouched(tmp_path):
    ...
    t = script(
        tmp_path, f'printf c > "$HOME/{token}"; printf c > "$XDG_CONFIG_HOME/{token}"\n'
    )
    try:
        assert run_it(target=t).returncode == 0  # same writes both runs → idempotent
        assert not (Path.home() / token).exists()  # redirected, not real HOME
    finally:
        leaked = Path.home() / token
        if leaked.exists():
            leaked.unlink()
...
def test_no_target_is_error(tmp_path):
    assert (
        subprocess.run(["bash", str(HELPER), "--"], capture_output=True).returncode == 2
    )
